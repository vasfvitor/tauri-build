...
#   scripts/run.sh --only baseline --os ubuntu-24.04 --runs 2 --tag warm-test
#   scripts/run.sh --only cache --twice # run the same selection twice back to back
#                                       # (second run = warm caches), both reports saved
#   scripts/run.sh --no-wait            # just dispatch and print the run URL
# --- end of usage ---
set -euo pipefail
cd "$(dirname "$0")/.."

ONLY="" OS="" RUNS="1" TAG="" WAIT=1 TWICE=0 REF="${REF:-}"
while [ $# -gt 0 ]; do
  case "$1" in
    --only) ONLY="$2"; shift 2 ;;
    --os)   OS="$2";   shift 2 ;;
...
# Find the run created by our dispatch: the workflow's display title carries
# no inputs, so poll for a run newer than the dispatch timestamp instead of
# trusting `--limit 1` right after `gh workflow run`.
find_run() {
  local since="$1" id="" i
...
dispatch_and_wait() {
  local tag="$1" since run_id dest
  since=$(date -u +%Y-%m-%dT%H:%M:%SZ)
  echo ">> dispatching experiments (only='$ONLY' os='$OS' runs=$RUNS tag=$tag ref=$REF)"
  gh workflow run experiments.yml --ref "$REF" \
    -f only="$ONLY" -f os="$OS" -f runs="$RUNS" -f tag="$tag"
  run_id=$(find_run "$since")
  echo ">> run $run_id: $(gh run view "$run_id" --json url -q .url)"
  [ "$WAIT" = 1 ] || return 0
  gh run watch "$run_id" --exit-status || echo ">> some jobs failed (report still collected)"
  dest="results/${tag}"
  mkdir -p "$dest"
  gh run download "$run_id" --name report --dir "$dest" || echo ">> no report artifact"
  gh run download "$run_id" --pattern 'timing--*' --dir "$dest/raw" || echo ">> no timing artifacts"
...
dispatch_and_wait "$TAG"
if [ "$TWICE" = 1 ]; then
  dispatch_and_wait "${TAG}-warm"
fi
